#include "sdf_map/sdf_map.h"
#include "sdf_map/map_ros.h"
#include <sdf_map/raycast.h>
...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  ROS_INFO("Hit: %lf, Miss: %lf, Min: %lf, Max: %lf, Thresh: %lf", mp_->prob_hit_log_, mp_->prob_miss_log_, mp_->clamp_min_log_, mp_->clamp_max_log_, mp_->min_occupancy_log_);
...
void SDFMap::clearAndInflateLocalMap() {
  ...
  if (mp_->virtual_ceil_height_ > -0.5) {
    ...
    // ROS_ERROR("Ceil Height: %lf, Ceil ID: %d", mp_->virtual_ceil_height_, ceil_id);
    for (int x = md_->local_bound_min_(0); x <= md_->local_bound_max_(0); ++x)
      for (int y = md_->local_bound_min_(1); y <= md_->local_bound_max_(1); ++y) {
        // md_->occupancy_buffer_inflate_[toAddress(x, y, ceil_id)] = 1;
        md_->occupancy_buffer_[toAddress(x, y, 0)] = mp_->clamp_max_log_;
